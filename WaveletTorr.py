...
class WaveletTorr(object):
    ...
    def wavelet(Y, dt, pad=0, dj=-1, s0=-1, J1=-1, mother=-1, param=-1):
    	...
    	kplus = np.arange(1, np.floor(n / 2 + 1))
    ...
    def wavelet_calc(data, dt, hrra, hrra2, powers):
        ...
        #        break
        #    except:
        #        comp = input('Component? (bx, by, bz, bp, ea, er) ')
    ...
    #    rec_text = hrra2 - 0.7
        ...
        n = len(sst)
        dt = dt
        time = np.linspace(hrra, hrra2 , len(sst))
        ...
        s0 = 2 * dt
        # if comp == 'er' or comp == 'ea':
        #     dt = dte
        #     j1 = 8 / dj
        # if comp == 'bx' or comp == 'by' or comp == 'bz' or comp == 'bp':
        #     dt = dtb
        #     j1 = 10 / dj
        ...
        signif = wave_signif(([1.0]), dt=dt, sigtest=0, scale=scale, lag1=lag1,
                             mother=mother)
        ...
        return (power, sst, period, time, coi)
